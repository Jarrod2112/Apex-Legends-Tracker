<template>
  <section>
      <div v-if="loading">
          <h3>Im Loading, Load your weapon ...</h3>
      </div>

        <!-- If error, displays specific error -->
       <div v-if="error">
          <h1>{{error}}</h1>
         <!-- Link back to home page -->
          <router-link to="/">Go back</router-link>
      </div>
        <!-- Displays selected data "Legend Name"-->
      <div v-if="profileData" class="container">
          <h1 class="gamertag">
             <!-- Receives and displays avatar logo. -->
...
async created() {
    ...
    try {
        ...
        // Stores all data received data in profilData Varaiable.
        // Comes in as objest called "data" so need data twice.
